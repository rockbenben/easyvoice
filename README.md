...
> **注意**：双击启动需要整合包（自带 `runtime/` 运行时与模型），预编译整合包见上方[下载](#下载)。想跑源码或自己打包（`build.ps1` 在源码仓库里），见 **[DEVELOPMENT.md](DEVELOPMENT.md)**。

### 第 3 步：浏览器中使用

启动后自动打开浏览器 `http://127.0.0.1:7860`（该端口被占用时会自动改用备用端口，以命令行窗口里显示的地址为准），共四个标签页：
